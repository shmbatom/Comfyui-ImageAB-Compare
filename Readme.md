Comfyui-ImageAB-Compare - ComfyUI 二图对比查看插件
...
将 image\_ab\_compare.py 放入 ComfyUI/custom\_nodes/ 目录

将 tupian\_duibi.js 放入 ComfyUI/custom/js/ 目录

（可选）如需自定义样式，将 custom.css 放入 ComfyUI/custom/css/ 目录
...
支持 ComfyUI Manager 的用户可直接在管理器中搜索 ImageABCompare 安装（需先发布到管理器仓库）
